...
\item Depending on the context, both underline or/and acute letters can also refer to row as well. 
...
In this thesis \textit{computational kernels} define the parts of an algorithm with the highest costs. These costs include both arithmetic and communication. 
...
Since $\{r_0, Ar_0, \ldots, A^{k-1}r_0\}$ is usually ill-conditioned the Arnoldi method is incorporated to produce $k + 1$ orthonormal basis vectors $\underline{Q}_k = [q_1, q_2, \ldots, q_k, q_{k + 1}]$ with $q_1 = r_0/\norm{r_0}_2$ and a $k + 1 \times k$ upper Hessenberg coefficient matrix $\underline{H}_k$ where
\begin{equation*}
	AQ_k = \underline{Q}_k\underline{H}_k.
\end{equation*}
With these conditions $z_k$ can be defined as $z_k = Q_ky$ such that
\begin{eqnarray*}
	\text{min}_{z_k} \norm{b - A(x_0 + z_k)}_2 &=& \text{min}_y \norm{r_0 - AQ_ky}_2 \\
	 &=& \text{min}_{y} \lVert r_0 - \underline{Q}_k \underline{H}_k y \rVert_2.
\end{eqnarray*}
Since $q_1 = r_0/\norm{r_0}_2$ and $\underline{Q}_k$ is orthonormal, one has
\begin{eqnarray} 
	\text{min}_y \lVert r_0 - \underline{Q}_k \underline{H}_k y \rVert_2 &=& \text{min}_{y} \lVert \underline{Q}^T_k r_0 - \underline{H}_k y \rVert_2 \nonumber \\ \label{eq:stdgmreslsp}
	&=& \text{min}_{y} \norm{\beta e_1 - \underline{H}_k y}_2 
\end{eqnarray}
with $\beta = \norm{r_0}_2$.
$\underline{H}_k$ is then factored as $\underline{H}_k = \underline{G}_k \underline{R}_k$ with $\underline{G}_k$ being a product of $k$ Givens rotations and $\underline{R}_k$ being upper triangular. The upper triangular system to solve is then given by
\begin{equation*}
	\text{min}_y \norm{\beta \underline{G}^T_k e_1 - \underline{R}_k y}_2
\end{equation*}
The solution can then be obtained by computing $x_k = x_0 + Q_ky$. Note that the absolute value of the last coordinate of $\beta \underline{G}^T_k e_1$ is $\norm{b - Ax_k}_2$, the absolute residual at iteration $k$. 
...
\caption{Newton CA-GMRES} % give the algorithm a caption
