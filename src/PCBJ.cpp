...
sparse_status_t PCBJ::mv(double *x, double *y, struct matrix_descr descr) {
...
sparse_status_t PCBJ::precondition(double *x) {
...
sparse_status_t PCBJ::setUp() {
	...
	mkl_sparse_copy (*A_mkl, descr, M_mkl);
	
	mkl_sparse_convert_bsr(*M_mkl, 3, SPARSE_LAYOUT_ROW_MAJOR, SPARSE_OPERATION_NON_TRANSPOSE, &B);
	mkl_sparse_destroy(*M_mkl);
	descr.type = SPARSE_MATRIX_TYPE_BLOCK_DIAGONAL;
	mkl_sparse_copy (B, descr, M_mkl);
	mkl_sparse_destroy(B);
	sparse_index_base_t indexing = SPARSE_INDEX_BASE_ZERO;
	sparse_layout_t block_layout = SPARSE_LAYOUT_ROW_MAJOR;
	size_t rows, cols, block_size, *rows_start, *rows_end, *col_indx;
	double *values;
	
	mkl_sparse_d_export_bsr(*M_mkl, &indexing, &block_layout, &rows, &cols, &block_size, &rows_start, &rows_end, &col_indx, &values);
