...
v3 = (A-ritz(2)*eye(m))*v2;
v4 = (A-ritz(3)*eye(m))*v3;
V0_accute = [v2, v3, v4];
...
H0 = h0_blackletter;
ritz = eig(H_ref(4:6,4:6));

B1_ = eye(3);
B1_ = [ritz(1) 0 0; B1_];
B1_(2,2) = real(ritz(2));
B1_(3,3) = real(ritz(2));
B1_(2,3) = - imag(ritz(2))^2;
B1 = B1_(1:3,1:3);

v4 = Q0_(:,4);

v5 = (A - ritz(1)*eye(m))*v4;
v6 = (A - real(ritz(2)*eye(m)))*v5;
v7 = (A - real(ritz(2))*eye(m))*v6 + imag(ritz(2))^2*v5;
...
%strange thing to do !!??
Q1_accute = Q1_accute * (-1);
R1_accute = R1_accute * (-1);
