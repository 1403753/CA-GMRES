...
sparse_status_t ILU0_ca::setUp() {
	...
	const Mtx_CSR *A_mtx = ctx->getA_mtx();
	size_t n = A_mtx->n;
	size_t nz = A_mtx->nz;
	Mtx_CSR dest{n, nz};
	sparse_matrix_t *A_mkl = ctx->getA_mkl();
	...
	at_plus_a(ctx->getA_mtx(), &bnz, &b_row_ptr, &b_col_indx);

	std::cout << " SIZE: " << nParts << std::endl;
	...
	permute_Mtx(A_mtx, &dest, part, part);
	
	mkl_sparse_destroy(*A_mkl);
	
	mkl_sparse_d_create_csr(A_mkl, SPARSE_INDEX_BASE_ZERO, dest.n, dest.n, dest.row_ptr, dest.row_ptr + 1, dest.col_indx, dest.values);
	mkl_sparse_order(*A_mkl);
	ctx->setOperator(A_mkl);
